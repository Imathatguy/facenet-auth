...
from defend_faces import generate_protection_noise, split_and_combine
...
def main_run(selection, ratio):
    ...
    if resample_users:
        ...
        random_data = np.load(random_loc)

        n_feat = 512

        a = FaceUserPopulation(embeddings_loc, labels_loc,
                               label_str_loc, n_feat)

        a.normalize_data()
        a.split_user_data(0.3)

        random_vdata = a.normalize_external(random_data)
    ...
    path = os.path.join(tmp_path, "models", "face")
    mkdir_p(path)
    ...
    for u in sorted(a.users.keys()):
        ...
        target_data, other_data = generate_protection_noise(target_data,
                                                            other_data, ratio)
        rand_data, other_data = split_and_combine(random_vdata,
                                                  other_data,
                                                  target_data.shape[0])
...
if __name__ == "__main__":
    ...
    for _ in range(1):
        ...
        mkdir_p("./user_defense_results/{}_probaresults/".format(clf_titles[selection]))

        results_holder.append(main_run(selection, ratio))
    tmp_file = tempfile.mkstemp(suffix=".pickle",
                                dir="./user_defense_results/{}_probaresults/".format(
